...
Date::Date(int year, int month , int day, int hour, int min, int sec) : year(year) ,
 month(month) , day(day), hour(hour) , min(min), sec(sec){}

Date::Date(int hour , int min, int sec): year(0) ,
 month(0) , day(0), hour(hour) , min(min), sec(sec){}

// TODO: implement the < comparison operator
bool Date::operator<(const Date& rhs) const {
  if (this->hour < rhs.hour) { return true; }
	if (this->hour > rhs.hour) { return false; }
	if (this->min < rhs.min) { return true; }
